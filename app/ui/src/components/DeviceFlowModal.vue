...
<template>
  <Modal :modelValue="true" @update:modelValue="!$event && deviceFlowModal?.onCancel()" :title="t('github_auth.title')" icon="mdi:github" iconColor="text-black dark:text-white" width="500px">
        <div class="p-8 flex flex-col items-center text-center">
            ...
            <p class="text-sm text-neutral-400 mb-6 font-medium leading-relaxed">
            ...
            <div class="flex justify-center flex-nowrap gap-1.5 my-2 relative group w-full" @click="copyCode" :title="t('github_auth.copy_code_tooltip')">
                <template v-for="(char, idx) in codeChars" :key="idx">
                    ...
                    <div v-else class="w-9 h-11 flex items-center justify-center bg-[#2a2a2d] border border-neutral-700/50 rounded-lg text-lg font-mono font-bold text-white shadow-inner">
                        {{ char }}
                    </div>
                </template>
            ...
            <p class="text-xs text-neutral-400 mt-5 flex items-center gap-2 justify-center bg-black/20 py-2 px-4 rounded-full border border-neutral-800">
