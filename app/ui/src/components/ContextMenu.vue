      ...
      <template v-for="(item, index) in items" :key="index">
        ...
        <div v-else 
          ...
          <div v-if="item.shortcut || item.subItems?.length" class="text-neutral-500 flex items-center gap-2">
            ...
            <Icon v-if="item.subItems?.length" icon="lucide:chevron-right" class="text-neutral-400" />
          </div>
          </button>
          
          <!-- Submenu -->
          <div v-if="item.subItems?.length && hoverIndex === index" 
               class="absolute left-full top-0 bg-[#2D2D2D] border border-neutral-700 min-w-[220px] rounded shadow-xl py-1 flex flex-col text-[#E0E0E0] text-xs pointer-events-auto ml-1 z-50">
            <template v-for="(subItem, subIndex) in item.subItems" :key="'sub_'+subIndex">
              <div v-if="subItem.separator" class="h-px bg-neutral-700 my-1 mx-2"></div>
              <button v-else @click.stop="handleItemClick(subItem)" 
                      class="flex items-center justify-between px-4 py-1.5 w-full text-left"
                      :class="[
                        subItem.disabled ? 'opacity-50 cursor-not-allowed text-neutral-500' : 'hover:bg-[#4A4A4A] cursor-pointer text-[#E0E0E0]'
                      ]">
                <div class="flex items-center gap-2 flex-1">
