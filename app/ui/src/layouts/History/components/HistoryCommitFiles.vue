     ...
     <div class="flex-1 flex overflow-hidden h-full">
        <div class="border-r border-line bg-app flex-shrink-0 relative h-full flex flex-col overflow-hidden" :style="{ width: layoutRefs.historyDetailTreeWidth.value + 'px' }">
           ...
           <div v-else-if="isLoading" class="p-8 text-center">
               <Icon icon="lucide:loader-2" class="animate-spin text-accent mx-auto mb-2" />
               <span class="text-[9px] text-content-muted uppercase font-bold tracking-widest">{{ t('history_detail.loading_tree') }}</span>
           </div>
           <div v-else class="p-8 text-center text-content-muted uppercase text-[9px] font-bold tracking-widest opacity-50">
