...
const emit = defineEmits(['toggle', 'refreshPRs', 'createPR', 'addRemote']);
</script>

<template>
  <div class="px-2 py-1 flex items-center gap-1.5 text-xs font-bold text-content-muted cursor-pointer hover:bg-surface-hover transition-colors group h-7"
    ...
    <div v-if="item.id === 'remotes'" class="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
      <button @click.stop="emit('addRemote')" class="p-1 hover:bg-surface-hover rounded transition-colors group/btn" :title="'Add remote'">
    ...
    <div v-if="item.id === 'pull_requests' && hasActivePRProvider" class="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
      <button @click.stop="emit('refreshPRs')" class="p-1 hover:bg-surface-hover rounded transition-colors group/btn">
        <Icon icon="lucide:rotate-cw" :class="{'animate-spin text-accent': isPRLoading}" class="text-[10px]" />
      </button>
      <button @click.stop="emit('createPR')" class="p-1 hover:bg-surface-hover rounded transition-colors group/btn ml-1">
        <Icon icon="lucide:plus" class="text-[10px] group-hover/btn:text-accent" />
      </button>
    </div>
  </div>
